...
- **`park`**: command dispatches, sends a REFER to the parking lot's
  pilot extension (confirmed correct on this PBX — `asterisk -rx
  "dialplan show 70@from-internal"` shows it reaches `Park()`), and the
  synchronous `result`/`park` confirmation events fire correctly — **but
...
| `{"cmd":"park","ext":"70","call_id":"..."}` | **New in v1.3.** Parks a call by blind-transferring it (REFER, the exact same `call_transfer()` mechanism `blind_transfer` already uses — parking a call *is* a blind transfer, just to a specific kind of PBX resource) to `ext` — the target parking lot's **pilot** extension (e.g. this engine's own test PBX's `"70"`, which reaches Asterisk's `Park()` app — confirmed read-only, see `core/E2E-F1.md` "F5 park"). `ext` is **required**, not defaulted to anything — a parking lot's pilot extension is per-PBX configuration (this repo is public; there's no universal-standard value to bake in, unlike `*43`/`*60` test codes, which this file never claimed `park` used either). Same target-address shape as `blf_subscribe`'s own `ext` (`sip:<ext>@<same PBX host the account registered against>`, built the same way — see `build_pbx_ext_uri()` in `ctrl_json.c`, shared by both). `call_id` is optional, same `resolve_call()` fallback-to-current-call convention as every other call-scoped command. **The resulting confirmation event's `ext` is always the *pilot* extension the request targeted, never a specific auto-assigned parking slot number** — see "Events" `park` row and this file's own top-of-file v1.3 status paragraph for exactly why that's not observable over plain SIP signaling this engine's call leg is party to, and for this command's current real-PBX e2e status (dispatches correctly; a client-side REFER-progress-subscription issue specific to targeting `Park()` was found and is not yet resolved — see `core/E2E-F1.md` "F5 park"). |
