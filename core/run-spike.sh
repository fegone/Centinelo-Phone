...
#
# audio_codecs (v1.6, codecs sprint): every entry below is
# "name/srate/ch", explicit, never a bare name - see BUILD.md "Findings"
# ("audio_codecs bare-name silent-fallback bug") and PROTOCOL.md
# "set_codecs" for the full writeup. Short version: baresip's own
# account.c audio_codecs= parser defaults a *bare* name to srate=8000/
# ch=1 before matching it against the real registered codec - which
# silently fails (a logged warning, not an error) for opus (48000/2,
# modules/opus/opus.c), and a fully-failed-to-match audio_codecs=
# string leaves the account's codec list empty, which baresip then
# treats as "no restriction" and falls back to the *entire* compiled-in
# codec catalog - the opposite of what this line asks for, with no
# error anywhere. Explicit srate/ch on every entry (opus AND pcmu/pcma,
# for consistency/self-documentation even though pcmu/pcma's own
# 8000/1 registration happens to match the parser's bare-name default)
# sidesteps this entirely. Order is preference: opus first (this
# spike's test endpoint allows `(opus|ulaw)` - see BUILD.md "Module
# selection" - so a real negotiation prefers it), G.711 as the
# universal fallback.
...
ACCOUNT_PARAMS=";auth_pass=${CENT_SECRET};mediaenc=dtls_srtp;medianat=ice;rtcp_mux=yes"
ACCOUNT_PARAMS="${ACCOUNT_PARAMS};audio_codecs=opus/48000/2,pcmu/8000/1,pcma/8000/1;regint=120"
...
# opus (v1.6, codecs sprint - matches the endpoint's allow=(opus|ulaw),
# opus preferred/wideband, g711 the universal fallback - see BUILD.md
# "Module selection" and "Codecs (opus)") + auconv/auresamp (format
...
# *after* g711/opus/ice/dtls_srtp or those restrictions silently fail to
# bind (found by running the spike: "account: medianat not found: 'ice'"
# etc. when account.so was first in the list).
