...
git apply --directory=core/deps/baresip core/patches/0006-baresip-msvc-arg-parsing.patch
```
...
failed instead of seeing nothing), and "`HAVE_GETOPT` is false on MSVC"
below for patch 0006 (`src/main.c`'s CLI argument parsing never compiled
on Windows at all — every argv the engine received, including the `-f
<scratch_dir>` this repo's own shell passes, was silently dropped).
...
### `HAVE_GETOPT` is false on MSVC — every CLI arg (`-f` included) was silently dropped on Windows (patch 0006)

**Root cause:** `src/main.c`'s *entire* option-parsing loop —
`getopt(argc, argv, "46a:de:f:p:hu:n:vst:m:Tc")` and its whole
`switch`, including `case 'f': conf_path_set(optarg); break;` — was
wrapped in `#ifdef HAVE_GETOPT`, with the `#else` branch doing nothing
but `(void)argc; (void)argv;`. `HAVE_GETOPT` comes from
`core/deps/re/cmake/re-config.cmake`'s `check_include_file(getopt.h
HAVE_GETOPT_H)`, which is false on every MSVC toolchain (no
`<getopt.h>` ships with MSVC). So on the Windows build, **every**
argument the engine's `argv` ever received — not just some — was a
no-op from the moment `main()` started.

**Confirmed against a real Windows machine (2026-08-13):** the shell
launches the engine with `.arg("-f").arg(&plan.scratch_dir)` (see
`shell/src-tauri/src/sidecar.rs`) — the scratch dir holding the
`config`/`accounts` the shell had just written, including that
config's `ctrl_json` module line. With `-f` a no-op, `conf_path_set()`
never ran, so `conf_path_get()` fell back to its Windows default,
`%APPDATA%\.baresip`, and the engine loaded (or autogenerated) whatever
was already there instead — no `module_path`, no `ctrl_json`, no
account. Manually copying this repo's own `config`/`accounts` into
`%APPDATA%\.baresip` and relaunching **fully unblocked the engine**:
WSS registration to the test PBX succeeded end to end (confirmed
extension-registered-by-WebSocket on the PBX side), and `ctrl_json`
loaded, restoring the shell↔engine control channel. That confirms the
argument-parsing gap was the *only* thing wrong on Windows — nothing
about the dual-transport/WSS code path itself is Windows-broken; it
just never received the config telling it to use WSS in the first
place.

**Fix (`core/patches/0006-baresip-msvc-arg-parsing.patch`):** rather
than duplicating `main()`'s ~90-line `switch`, the patch adds
`core/deps/baresip/src/cent_getopt.h` — a small, dependency-free
`getopt()`-alike (`cent_getopt()`/`cent_optarg`, both `static`,
`#define`d over `getopt`/`optarg` for this translation unit only) that
implements exactly the subset of POSIX `getopt()` semantics `main()`'s
optstring needs: single-dash short options, `':'` for "takes a value"
(glued — `-fpath` — or as the next `argv[]` — `-f path`), bundled
no-value flags (`-46`), `'?'` (not a crash, not a silent skip-to-end)
for anything unrecognized or missing its value, `"--"` to stop parsing.
`main.c` itself only changes by un-wrapping the `#ifdef HAVE_GETOPT`
around `usage()` and the `for (;;)` loop (both now compile
unconditionally) and adding the `#ifndef HAVE_GETOPT` `#include
"cent_getopt.h"` + two `#define`s right above. **Zero behavior change
on macOS/Linux** — `HAVE_GETOPT` is true there, so
`cent_getopt.h`/the `#define`s never compile in, and `getopt()`/`optarg`
still resolve to the real libc ones; confirmed by a syntax check and a
full rebuild with `-DHAVE_GETOPT` producing an identical `baresip`
binary behavior (`-f` still loads the given config path, verified
against the test PBX's config format locally).

**Unit tests (`core/patches/test/test_cent_getopt.c`):** standalone,
no baresip/re headers — includes `cent_getopt.h` directly and asserts
against `main.c`'s real optstring (`"46a:de:f:p:hu:n:vst:m:Tc"`):
`-f <path>` as two argv entries (the exact shape `sidecar.rs` sends),
`-fpath` glued, bundled `-46v`, an unknown flag followed by a valid one
(must not get stuck), a value-taking flag with nothing after it (must
not read out of bounds), `argc==1`, `"--"`, and a bare positional arg.
Compile and run directly, no CMake/ctest wiring needed (this header
only exists once the patch is applied to the submodule, which isn't
part of this repo's own CMake project):

```bash
clang -fsanitize=address,undefined -g -Wall -Wextra -std=c99 \
  core/patches/test/test_cent_getopt.c -o /tmp/test_cent_getopt
/tmp/test_cent_getopt
```

All 7 cases pass clean under ASan+UBSan (macOS, this spike).

**What to check on a real Windows build (this is the part that could
not be verified locally — no Windows machine here):**

1. Build with this patch applied (`git apply --directory=core/deps/baresip
   core/patches/0006-baresip-msvc-arg-parsing.patch`, same as the other
   patches) and launch the engine exactly as the shell does:
   `baresip.exe -f <scratch_dir>` (or run `core/run-spike.sh`'s
   equivalent — any `-f <path>` invocation).
2. **Unambiguous pass signal:** the engine's stderr banner line
   (`baresip v4.9.0 Copyright ...`, or nothing at all if
   `CENT_JSON_STDOUT` is set — see the `lg.enable_stdout` section
   above) is followed by `ctrl_json` actually starting — i.e. a
   `{"event":...}` / a `result` line comes back over stdout in response
   to a command written to stdin (see `PROTOCOL.md`). Under the bug,
   nothing at all would come back on stdout: `ctrl_json` never loaded
   (the scratch config's `module_path`/module list were never read),
   so there is no control channel to respond on stdin in the first
   place — the process would sit there having loaded whatever modules
   happened to be configured in `%APPDATA%\.baresip\config` instead
   (or none, if that file/dir doesn't exist yet).
3. **Unambiguous file-level check**, no engine instrumentation needed:
   after launching with `-f <scratch_dir>`, confirm
   `%APPDATA%\.baresip\` was **not** touched/created (its `config`/
   `accounts` — if present at all from before this fix — must have an
   mtime strictly older than this run) — the fixed engine has no reason
   to ever look there once `-f` correctly points `conf_path_get()` at
   the scratch dir. Under the bug, this directory is exactly where the
   engine was reading from/autogenerating into on every launch.
4. **Registration check against the test PBX** (the strongest signal,
   and how this bug was actually confirmed in the first place — see
   above): with a real account in the scratch dir's `accounts` file,
   `pjsip show endpoint 1100` (or the relevant test extension) on the
   PBX side should show it registered by WebSocket shortly after
   launch. Under the bug this never happens no matter how long you
   wait, because `account`'s module load and the account file itself
   both come from `%APPDATA%\.baresip`, not the scratch dir — usually
   `Populated 0 accounts` in whatever log output does reach a terminal
   (stderr, or stdout pre-`ctrl_json` if `CENT_JSON_STDOUT` is unset).
5. **Regression check, not a new one:** every other `-f`-independent
   thing already covered by Windows CI (`.github/workflows/core-build.yml`
   — plain compile + the existing unit tests) must stay green; this
   patch does not touch anything reachable when `-f` isn't passed at
   all other than un-wrapping two `#ifdef`s that were already
   unconditionally-false/no-op on that platform.
